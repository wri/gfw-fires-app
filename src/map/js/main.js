...
if (!babelPolyfill) {
  console.log(
    "Missing Babel Polyfill.  May experience some weirdness in IE < 9."
  );
}
...
let configureApp = () => {
  ...
  defaults.corsEnabledServers.forEach(server => {
...
let lazyloadAssets = () => {
  ...
  loadCSS(`https://js.arcgis.com/3.31/esri/css/esri.css`);
};
...
const parseTitles = (planetBasemaps, isMonthly) => {
  ...
  return planetBasemaps
    .filter(basemap => {
      if (
        basemap.title === "Latest Monthly" ||
        basemap.title === "Latest Quarterly"
      ) {
        return false;
      } else {
        return true;
...
const queryPlanet = new Promise((resolve, reject) => {
    ...
    if (this.readyState === 4) {
      if (this.status === 200) {
        ...
        for (let i = 0; i < layerCollectionLength; i++) {
          ...
          const title = currentLayer.getElementsByTagName("ows:Title")[0]
            .innerHTML;
        ...
        basemaps.forEach(function(basemap) {
          if (
            basemap &&
            basemap.hasOwnProperty("title") &&
            basemap.title.indexOf("Monthly") >= 0
          ) {
          ...
          if (
            basemap &&
            basemap.hasOwnProperty("title") &&
            basemap.title.indexOf("Quarterly") >= 0
          ) {
  ...
  xhttp.open(
    "GET",
    "https://api.planet.com/basemaps/v1/mosaics/wmts?api_key=6c3405821fb84e659550848226615428",
    true
  );
  xhttp.send();
});
...
const initializeApp = () => {
  ...
  ReactDOM.render(
    <CalendarModal calendars={defaults.calendars} />,
