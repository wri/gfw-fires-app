...
let LayersHelper = {
  ...
  performIdentify(evt) {
    ...
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyActive(mapPoint, layer));
      }
    }

    layer = app.map.getLayer(KEYS.activeFires48);
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyActive(mapPoint, layer));
      }
    ...
    layer = app.map.getLayer(KEYS.activeFires72);
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyActive(mapPoint, layer));
      }
    ...
    layer = app.map.getLayer(KEYS.activeFires7D);
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyActive(mapPoint, layer));
      }
    ...
    layer = app.map.getLayer(KEYS.activeFires1Y);
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyActive(mapPoint, layer));
      }
    ...
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyViirs(mapPoint, layer));
      }
    }

    layer = app.map.getLayer(KEYS.viirsFires48);
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyViirs(mapPoint, layer));
      }
    ...
    layer = app.map.getLayer(KEYS.viirsFires72);
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyViirs(mapPoint, layer));
      }
    ...
    layer = app.map.getLayer(KEYS.viirsFires7D);
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyViirs(mapPoint, layer));
      }
    ...
    layer = app.map.getLayer(KEYS.viirsFires1Y);
    if (layer) {
      if (layer.visible) {
        deferreds.push(Request.identifyViirs(mapPoint, layer));
      }
          ...
          switch (item.layer) {
            case KEYS.activeFires:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.activeFires)
              );
              break;
            case KEYS.viirsFires:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.viirsFires)
              );
              break;
            case KEYS.archiveFires:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.archiveFires)
              );
              break;
            case KEYS.noaa18Fires:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.noaa18Fires)
              );
              break;
            case KEYS.burnScars:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.burnScars)
              );
              break;
            case KEYS.oilPalm:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.oilPalm)
              );
              break;
            case KEYS.rspoOilPalm:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.rspoOilPalm)
              );
              break;
            case KEYS.woodFiber:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.woodFiber)
              );
              break;
            case KEYS.mining:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.mining)
              );
              break;
            case KEYS.gfwLogging:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.gfwLogging)
              );
              break;
            case KEYS.loggingConcessions:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.loggingConcessions)
              );
              break;
            case KEYS.oilPalmGreenpeace:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.oilPalmGreenpeace)
              );
              break;
            case KEYS.woodFiberGreenpeace:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.woodFiberGreenpeace)
              );
              break;
            case KEYS.loggingGreenpeace:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.loggingGreenpeace)
              );
              break;
            case KEYS.coalConcessions:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.coalConcessions)
              );
              break;
            case KEYS.protectedAreasHelper:
              features = features.concat(
                this.setActiveTemplates(
                  item.features,
                  KEYS.protectedAreasHelper
                )
              );
              break;
            case KEYS.fireStories:
              features = features.concat(
                this.setStoryTemplates(item.features, KEYS.fireStories)
              );
              // features = features.concat(this.setActiveTemplates(item.features, KEYS.fireStories));
              break;
            case KEYS.overlays:
              features = features.concat(
                this.setActiveTemplates(item.features, KEYS.overlays)
              );
              break;
            case KEYS.boundingBoxes:
              features = features.concat(
                this.setDigitalGlobeTemplates(item.features)
              );
              break;
            default:
              // Do Nothing
  ...
  setActiveTemplates: function(featureObjects, keyword) {
    ...
    featureObjects.forEach(item => {
      ...
      if (
        ...
        keyword === KEYS.oilPalm ||
        keyword === KEYS.mining ||
        keyword === KEYS.loggingConcessions ||
        keyword === KEYS.oilPalmGreenpeace ||
        keyword === KEYS.woodFiberGreenpeace ||
        keyword === KEYS.loggingGreenpeace ||
        keyword === KEYS.coalConcessions
      ) {
        ...
        keyword === KEYS.rspoOilPalm ||
        keyword === KEYS.protectedAreasHelper
      ) {
        fire_results = this.getFirePopupContent(item);
        subscribe =
          '</table><div title="close" class="infoWindow-close close-icon"><svg viewBox="0 0 100 100"><use xlink:href="#shape-close" /></use></svg></div><div class="layer-subscribe-container"><button data-url=' +
  ...
  showLayer(layerObj) {
    ...
    if (layerObj.layerId === KEYS.digitalGlobe) {
      ...
      layerObj.layerId === KEYS.protectedAreas ||
      layerObj.layerId === KEYS.protectedAreasHelper
    ) {
      let level = 6,
        mainLayer = app.map.getLayer(KEYS.protectedAreas),
        helperLayer = app.map.getLayer(KEYS.protectedAreasHelper);
  ...
  hideLayer(layerId) {
    ...
    if (layerId === KEYS.digitalGlobe) {
      ...
      layerId === KEYS.protectedAreas ||
      layerId === KEYS.protectedAreasHelper
    ) {
  ...
  updateViirsArchiveDates(clauseArray) {
    ...
    const viirsLayer = app.map.getLayer(KEYS.viirsFires1Y);
    if (viirsLayer) {
  ...
  updateTreeCoverDefinitions(densityValue) {
    ...
    let rasterFunction = rasterFuncs.getColormapRemap(
