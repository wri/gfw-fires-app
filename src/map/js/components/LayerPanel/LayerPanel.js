...
export default class LayerPanel extends React.Component {
    ...
    return layer => {
      ...
      switch (layer.id) {
        case KEYS.waterStress:
          childComponent = (
            <WaterStressLegend url={layer.url} layerIds={layer.layerIds} />
          );
          break;
        case KEYS.sediment:
          childComponent = (
            <SedimentLegend url={layer.url} layerIds={layer.layerIds} />
          );
          break;
        case KEYS.majorDams:
          childComponent = (
            <DamsLegend url={layer.url} layerIds={layer.layerIds} />
          );
          break;
        ...
        case KEYS.noaa18Fires:
          childComponent = (
            <NoaaControls options={layer.calendar} loaded={this.props.loaded} />
          );
          break;
        case KEYS.burnScars:
          childComponent = (
        ...
        case KEYS.primaryForests:
          childComponent = (
            <ForestControls loaded={this.props.loaded} {...this.state} />
          );
          break;
        case KEYS.rspoOilPalm:
          childComponent = (
            <RSPOLegend url={layer.url} layerIds={layer.layerIds} />
          );
          break;
        case KEYS.peatlands:
          childComponent = (
            <LandCoverLegend url={layer.url} layerIds={layer.layerIds} />
          );
          break;
        case KEYS.fireHistory:
          childComponent = (
            <FireHistoryTimeline {...this.state} loaded={this.props.loaded} />
          );
          break;
        case KEYS.fireWeather: //fireRisk ??
          childComponent = (
            <RiskControls options={layer.calendar} loaded={this.props.loaded} />
          );
          break;
        case KEYS.lastRainfall:
          childComponent = (
            <RainControls options={layer.calendar} loaded={this.props.loaded} />
          );
          break;
        case KEYS.airQuality:
          childComponent = (
            <AirControls options={layer.calendar} loaded={this.props.loaded} />
          );
          break;
        case KEYS.windDirection:
          childComponent = (
            <WindControls options={layer.calendar} loaded={this.props.loaded} />
          );
          break;
        ...
        case KEYS.plantationTypes:
          childComponent = (
            <PlantationControls loaded={this.props.loaded} {...this.state} />
          );
          break;
        default:
          childComponent = null;
  ...
  render() {
    ...
    let landUseLayers = layersConfig
      .filter(l => l.group === 'forestUse')
      .map(this.checkboxMap('forestUse'), this);
    let fireRiskLayers = layersConfig
      .filter(l => l.group === 'fireRisk')
      .map(this.checkboxMap('fireRisk'), this);
    let conservationLayers = layersConfig
      .filter(l => l.group === 'conservation')
      .map(this.checkboxMap('conservation'), this);
    let landCoverLayers = layersConfig
      .filter(l => l.group === 'landCover')
      .map(this.checkboxMap('landCover'), this);
    if (app.mobile() === true && this.state.layerPanelVisible === false) {
      className += ' hidden';
    }
    if (
      this.state.panelsHidden === true &&
      ...
      <div className={className}>
        ...
        <div className="mobile-show">
          <div className="layer-category">
            <div className="layer-category-label pointer">Basemaps</div>
            <div>
              <div className="flex flex-wrap flex-justify-between padding">
                <div
                  className="basemap-item narrow pointer"
                  onClick={this.clickedBasemap.bind(this, KEYS.darkGrayBasemap)}
                >
                  <div
                    className={`basemap-thumbnail dark-gray-basemap ${
                      this.state.activeBasemap === KEYS.darkGrayBasemap
                        ? 'active'
                        : ''
                    }`}
                  />
                  <div className="basemap-label narrow">
                    {controlPanelText.darkGrayBasemap}
                  </div>
                </div>
                <div
                  className="basemap-item narrow pointer"
                  onClick={this.clickedBasemap.bind(this, KEYS.topoBasemap)}
                >
                  <div
                    className={`basemap-thumbnail topo-basemap ${
                      this.state.activeBasemap === KEYS.topoBasemap
                        ? 'active'
                        : ''
                    }`}
                  />
                  <div className="basemap-label narrow">
                    {controlPanelText.topoBasemap}
                  </div>
                </div>
                <div
                  className="basemap-item narrow pointer"
                  onClick={this.clickedBasemap.bind(this, KEYS.wriBasemap)}
                >
                  <div
                    className={`basemap-thumbnail wri-basemap ${
                      this.state.activeBasemap === KEYS.wriBasemap
                        ? 'active'
                        : ''
                    }`}
                  />
                  <div className="basemap-label narrow">
                    {controlPanelText.wriBasemap}
                  </div>
                </div>
                <div
                  className="basemap-item narrow pointer"
                  onClick={this.clickedBasemap.bind(this, KEYS.imageryBasemap)}
                >
                  <div
                    className={`basemap-thumbnail imagery-basemap ${
                      this.state.activeBasemap === KEYS.imageryBasemap
                        ? 'active'
                        : ''
                    }`}
                  />
                  <div className="basemap-label narrow">
                    {controlPanelText.imageryBasemap}
                  </div>
                </div>
                <div
                  className="basemap-item narrow pointer"
                  onClick={this.clickedBasemap.bind(this, KEYS.osmBasemap)}
                >
                  <div
                    className={`basemap-thumbnail osm-basemap ${
                      this.state.activeBasemap === KEYS.osmBasemap
                        ? 'active'
                        : ''
                    }`}
                  />
                  <div className="basemap-label narrow">
                    {controlPanelText.osmBasemap}
                  </div>
                </div>
              </div>
            </div>
