    ...
    return <div>
      <div className='timeline-container relative plantations'>
        ...
        <div className={`plantations-legend-container-type ${this.props.plantationSelectIndex === 1 ? '' : ' hidden'}`}>
          <LandCoverLegend url='http://gis-gfw.wri.org/arcgis/rest/services/land_use/MapServer' layerIds={[plantationOptions[1].value]} />
        </div>
        <div className={`plantations-legend-container ${this.props.plantationSelectIndex === 0 ? '' : ' hidden'}`}>
          <LandCoverLegend url='http://gis-gfw.wri.org/arcgis/rest/services/land_use/MapServer' layerIds={[plantationOptions[0].value]} />
        </div>
      </div>
    </div>;
  }
