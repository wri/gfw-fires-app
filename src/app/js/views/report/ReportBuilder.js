      ...
      renderer.execute(generateParams, function (customRenderer) {
		...
		queryCompanyConcessions: function () {
			...
			queryTask.execute(query, function (res) {
				...
				oilPalm = arrayUtils.filter(oilPalm, function (feature) { return feature.attributes.fire_count !== 0; });
				woodFiber = arrayUtils.filter(woodFiber, function (feature) { return feature.attributes.fire_count !== 0; });
				logging = arrayUtils.filter(logging, function (feature) { return feature.attributes.fire_count !== 0; });
				// render the tables
