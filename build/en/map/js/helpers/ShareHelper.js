  ...
  var ShareHelper = {
    applyStateFromUrl: function applyStateFromUrl(state) {
      ...
      if (activeLayers) {
        ...
        if (layerIds.indexOf(_constants2.default.loss) === -1) {
          _LayerActions.layerActions.removeActiveLayer(_constants2.default.loss);
        }

        // Gain is on by default, we need to turn it off if not present in the shared state
        if (layerIds.indexOf(_constants2.default.gain) === -1) {
          _LayerActions.layerActions.removeActiveLayer(_constants2.default.gain);
        }
      }
    ...
    prepareStateForUrl: function prepareStateForUrl() {
      ...
      if (activeBasemap !== _constants2.default.wriBasemap) {
